...
const char *const WG_CONF_PATH = "/etc/wireguard";
const char *const WG_CONF_SUFFIX = ".conf";
...
// return matched string at post=N in second array
...
int shell_stop_current(const char *cur_use) {
  char cmd[256];
...
int main(int agrc, const char **argv) {
  ...
  if (list_all_confs(&confs) < 0) return EXIT_FAILURE;
  strings_sort(confs);

  struct strings ifs;
  if (list_all_ifs(&ifs) < 0) return EXIT_FAILURE;
  ...
  // interactive
  print_current(confs, cur_conf_at);
  int next_conf_at = read_new_conf(confs);
  if (next_conf_at == cur_conf_at) {
  ...
  if (cur_conf_at >= 0) {
    printf("----- shutdown ------\n");
    shell_stop_current(cur_use_ref);
    printf("---------------------\n");
  }
  if (next_conf_at >= 0) {
    const char *next_use_ref = confs.ptr[next_conf_at];
    printf("----- configure -----\n");
